###########################################################
# rs_review_mentorsチャンネルの投稿を自動レビューします
###########################################################
...
# チェックするslackチャンネルのID
CHECK_CHANNEL_ID = 'C06VD256GR4' #rs_review_mentors

# チェック結果の送信先slackチャンネルのID
SEND_CHANNEL_ID = 'C06VD256GR4' #rs_review_mentors
        ...
        try:
            ...
            if len(reviews) > 0:
                ...
                for review in reviews:
                    ...
                    phase = fallback_lines[2][4:]
                    if phase == "ProtoSpace":
                        CLIENT.chat_postMessage(channel=SEND_CHANNEL_ID, thread_ts=oldest, text="protospaceのため手動でのチェックをお願いします", icon_emoji=':robot_face:', username='自動レビューbot')
                        continue
